...
namespace JigLibX.Physics
{
    ...
    public class ConstraintMaxDistance : Constraint
    {
        ...
        public override bool Apply(float dt)
        {
            ...
            // early out
            float clampedRelPos0MagSq = clampedRelPos0.LengthSquared();
            if (clampedRelPos0MagSq < JiggleMath.Epsilon)
                return false;
            
            // clamp position
            if (clampedRelPos0MagSq > mMaxDistance * mMaxDistance)
            {
                clampedRelPos0.Normalize();
                #region REFERENCE: clampedRelPos0 *= mMaxDistance;
                Vector3.Multiply(ref clampedRelPos0, mMaxDistance, out clampedRelPos0);
                #endregion
            }
            ...
            // early out
            if (Vr.LengthSquared() < minVelForProcessing * minVelForProcessing)
                return false;
            ...
            Vector3 N = Vr;
            N.Normalize();
            Vector3.Dot(ref Vr, ref N, out normalVel);
